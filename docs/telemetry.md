...
3. **The verdicts that are successes for Oban and non-events for the
   chart.** A fired timer whose execution is no longer live is *discarded* per
   ...
   a completed invocation delivered into a dead execution, for the spec 6.3 and
...
- **The identity key is `scope`, not `session_id`.** `StatifierOban.Timer.Key`
  ...
  hosts this package exists for. The correspondence is nonetheless one-to-one
  where both exist, and the bridge maps `scope` onto `statifier.session_id` for
...
whatever a host's own execution store calls the not-live case. This is the spec
6.2 verdict as data, and it is the value Oban buries inside `:result`.
...
entries the execution's datamodel holds; reading this one alone and expecting
that total is the mistake the split invites.
...
- **Scheduling-seam events land on the macrostep span, through the bridge's
  ...
  process - so a `scope` that is a host's durable execution id matches nothing
  and the event becomes its own span instead. No propagation machinery is
  involved either way.
...
- **Delivery-seam events are detached roots carrying a link, not spans inside
  ...
  Correlation back to the execution is by the `statifier.session_id` attribute
...
- **The fan-out events are what makes a fan-out's children reachable.**
  ...
  bridge opens a linked root on, so a child execution's own spans have a root
  to nest under and the arming trace is reached by the same `caller_context`
...
table from the persisted position, where `caller_context` is an optional entry
key; the Oban row survives that death intact either way, so a process-less host
