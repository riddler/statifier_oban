defmodule StatifierOban.Invoke.ChildStarter do
  @moduledoc """
  The seam a fan-out's child executions are created through.

  This package schedules fan-out child starts (ADR-0007) and does not
  create executions: creating a child execution is a durable,
  multi-execution operation behind a storage adapter this package has no
  dependency on. So the starter is a host-wired seam, exactly as
  `:invoke_delivery` is for the other direction: the host names a module
  here, and `StatifierOban.Invoke.ChildStartWorker` calls it once per
  index.

  A host running `statifier_persistence` wires the start-with-index
  function that package ships for this; a host with its own execution
  store wires its own. Nothing in this package knows which.

  ## Idempotency

  `c:start_child/5` is called from an Oban job, so it is called **at
  least once** per index and MUST be idempotent on
  `{parent_execution_id, invoke.invoke_id, index}` - the triple the
  child's identity is derived from. A second call for an index whose child
  ...
  environment-shaped failures an execution store has (the database is
  down, a lock is held). A child that can never be created exhausts the
  job's retries and is observable on the job row, and the index then has
  no execution record - the partially-started fan-out `sb-ADR-0009`
  decision 8 and `ADR-0007` decision 2 both specify behaviour for.
  Noticing it is the settlement side's, not this package's.
  """
  ...
  parent execution named by `parent_execution_id`.

  `parent_execution_id` is the scope the invocation was enqueued under -
  the same value that rides the invoke job's `"scope"` arg. `invoke` is
  the effect the fan-out invocation planned, byte-identical to the one
