defmodule StatifierOban.Invoke.Delivery do
  @moduledoc """
  The delivery seam a completed invoke goes through - the execution-liveness
  ...
  work, the result has to travel back into the execution as
  `done.invoke.<invoke_id>` with `donedata`. A durable job survives the
  execution it was started for, so before feeding anything back the host
  MUST establish the execution is still live and discard otherwise - a
  completed invoke against a dead or halted execution is discarded
  exactly the way a fired timer is. The guarantee is enforced here, at
  delivery time.
  ...
  positions answers it from its own execution store and feeds the completion
  ...
  chosen by the engine, one drive at a time, out of that execution's
  ...
  `done.invoke.<invoke_id>` back and the execution transitions into
  *another* invoking state, the `<invoke>` entered there is planned by the
  drive this delivery caused - so its handler comes from whatever registry
  that drive was given. A `Statifier.Session` host fixed one at
  `start_link/2` and every step sees it. A process-less host supplies one
  per drive, and the re-entry drive is a drive like any other: give it the
  same map the execution started with, or the second `<invoke>` raises
  `error.execution` at plan time on an unregistered `type` while the first
  step looked perfectly healthy. The seam is not the place to fix that,
  because the seam was never asked.
  ...
  Both run the same execution-liveness check first, because a durable job
  outlives the execution it was started for either way.
  ...
  then, reports the invocation named `invoke_id` complete with `donedata`.

  Returns `:delivered` when the completion was fed back, or `{:discarded,
  reason}` when the execution is not live and the completion was dropped.
  "Live" is stricter than "not terminated": a halted execution (`:done`
  and friends) still discards, because an event fed to a halted session
  just sits queued.
  ...
  persisting an execution, and a string is what an expression can usefully
  read.
  """
  ...
  is the ordinary case where the execution died before its invocation gave
  up; the job is discarded either way, so the caller records the outcome
  and does not retry on it.
